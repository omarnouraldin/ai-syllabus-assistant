  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (mode === 'pdf') {
        ...
        setProgress('🤖 Analyzing syllabus text...');
      ...
      setProgress('🧠 AI is extracting course information...');
      const result = await response.json();
      if (!response.ok) throw new Error(result.error || 'Upload failed');

      onSyllabusLoaded(result.data, result.rawText);
    ...
    <div className="container">
      <div style={{ maxWidth: 580, margin: '0 auto' }}>
        ...
        <div className="card">
          ...
          {/* Error */}
          {error && (
            <div style={{ marginTop: 14, padding: '10px 14px', background: '#fee2e2', color: 'var(--danger)', borderRadius: 'var(--radius-sm)', fontSize: '0.875rem' }}>
              ⚠️ {error}
            </div>
          )}
          ...
          {loading && progress && (
            <div style={{
              ...
              <span className="spinner spinner-dark" style={{ borderTopColor: 'var(--primary)', borderColor: 'rgba(99,102,241,0.2)' }} />
              {progress}
            </div>
          )}
